...
a_BBH = pymc.Uniform('a_BBH', 1e13, 1e15)    #semimajor axis at t
...
#Extinction
@pymc.deterministic
def Extinction(E_BV = E_BV):    #extinction=F0/F'
	R = 4.0                      #
	lamda_mu = lamda*1e6
	out=[]
...
def F_lamda(r_in = r_in, r_out = r_out, M_t = M_t, M_c_acc = M_c_acc):   #flux
	...
	def f(r):
		return 2*pi*r*(2*h*c**2*cosi/lamda**5)/(np.exp(h*c/(lamda*k_B*T_eff(r)))-1)
...
def f_lamda(r_s_out = r_s_out, r_s_in = r_s_in, M_s = M_s, M_s_acc = M_s_acc):   #flux
	...
	def g(r):
		return 2*pi*r*(2*h*c**2*cosj/lamda**5)/(np.exp(h*c/(lamda*k_B*t_eff(r)))-1)
...
@pymc.deterministic
def S(A=A, f_lamda = f_lamda, F_lamda = F_lamda, Extinction = Extinction):
	return A*(f_lamda+F_lamda)/Extinction

T = pymc.Normal('T', S , 1e32, value=spec, observed=True)
...
model = dict(a_BBH = a_BBH, q = q, M_t = M_t, M_s=M_s,
													...
													f_RS=f_RS, E_BV = E_BV, Extinction = Extinction,
													A = A, S = S, T = T)

M = pymc.MCMC(model)

M.sample(iter = 100000, burn = 10000, thin = 5) 
...
ax7.set_title('A')
ax8.hist(trace8[:], 25, histtype='step')
ax8.set_title('E_BV')
